  ...
  async connect(selectedLetter?: string): Promise<void> {
    ...
    try {
      ...
      const agent = new RealtimeAgent({
        ...
        instructions: (
          `أنت معلم أطفال للحروف الإنجليزية. الحرف الحالي هو ${selectedLetter || 'A'}.
استخدم أدوات الواجهة للتحكم في العناصر بدقة حسب تسلسل الدرس:
1) ui_show_letter ← للتعريف بالحرف
2) ui_show_words / ui_show_blackboard / ui_show_both حسب المرحلة
3) ui_show_song في نهاية الدرس
استخدم نبرة مشجعة فقط عند الإجابة الصحيحة، وقل "حاول مرة أخرى" عند الخطأ.
انتظر ردود الأطفال أثناء الأسئلة قبل الانتقال للمرحلة التالية.`
        ),
